...
if (siteSwiper) {
    ...
    document.querySelectorAll('.anchor-link').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            // Получаем ID из якорной ссылки
            const targetId = this.getAttribute('href').substring(1);
            const targetElement = document.getElementById(targetId);

            // Переключаемся на 6-й слайд
            mainSiteSwiper.slideTo(5, 0); // Слайды индексируются с 0

            // Прокручиваем до нужного элемента в 6-м слайде
            setTimeout(() => {
            ...
            }, 300); // Небольшая задержка, чтобы Swiper успел переключиться
        });
    });
